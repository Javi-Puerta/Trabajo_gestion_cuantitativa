...
class ModeloBase(ABC):
    def train(self, df: pd.DataFrame, feature_cols: list[str]) -> None:
        ...
        X = df[feature_cols]
        y = df["Target"]
        tscv = WalkForwardCV(n_splits=3, val_ratio=0.20)
        grid = GridSearchCV(estimator=self.clf, param_grid=self.param_grid,
...
class RandomForestModel(ModeloBase):
    def __init__(self, random_state: int = 42):
        ...
        self.param_grid = {
            ...
            "min_samples_leaf": [0.01, 0.05], "max_features": [None, "sqrt", 0.8, 0.5]
        }
...
class WalkForwardCV(BaseCrossValidator):
    ...
    def get_n_splits(self, X=None, y=None, groups=None):  # obligatorio por BaseCrossValidator
        return self.n_splits

    def _iter_test_indices(self, X=None, y=None, groups=None):  # obligatorio por BaseCrossValidator
